...
module.exports = async function (deployer: Deployer) {
  ...
  const configPath2 = `deploy/data/subgraph-output.json`;
  ...
  // const ids = [37, 60, 75];
  // const data = subnets.filter((a, i) => ids.includes(i));

  for (let i = 0; i < subnets.length; i++) {
    const subnet = subnets[i];
    const res = await contract.buildersPoolData(subnet.id);

    // const buildersDeposited = Number(formatUnits(res[1]));
    // const buildersVirtualDeposited = Number(formatUnits(res[2]));
    ...
    for (let k = 0; k < subnet.users.length; k++) {
      ...
      // const userDeposited = Number(formatUnits(userData[2]));
      // const userVirtualDeposited = Number(formatUnits(userData[3]));
...
// npx hardhat migrate --only 15
// npx hardhat migrate --network base --only 15
