...
module.exports = async function (deployer: Deployer) {
  ...
  // console.log(`wBTC before: ${await wBTC.balanceOf(user2)}`);
  // await wBTC.connect(user2).approve(pool, '100000000');
  // await pool.connect(user2).supply(wBTCAddress, '100000000', user2, 0);
  // console.log(`awBTC after supply: ${await awBTC.balanceOf(user2)}`);

  // await pool.connect(user2).withdraw(wBTCAddress, '99999999', user2);
  // console.log(`awBTC after withdraw: ${await awBTC.balanceOf(user2)}`);
  // console.log(`wBTC after: ${await wBTC.balanceOf(user2)}`);
};
