...
interface IChainLinkDataConsumer is IERC165 {
     ...
     */
    event DataFeedSet(string path, address[] feeds, uint64[] allowedPriceUpdateDelay);

    /**
     * @notice The event that is emitted when the price delay set.
     * @param feed The addresses of the feed.
     * @param allowedPriceUpdateDelay The maximum delay between the returned update time and the current time.
     */
    event PriceUpdateDelaySet(address feed, uint64 allowedPriceUpdateDelay);
     ...
     * @param feed_ The addresses of the feed.
     * @param allowedPriceUpdateDelay_ The delay in seconds.
     */
    function setAllowedPriceUpdateDelay(address feed_, uint64 allowedPriceUpdateDelay_) external;
     ...
     * @param allowedPriceUpdateDelays_ The maximum delay between the returned update time and the current time.
     * For example, if set to 120 seconds, the price returned by ChainLink should not be older than 120 seconds from the time of the call.
     */
